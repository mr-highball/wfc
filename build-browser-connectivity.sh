...
browser_directory="$repository_root/examples/passes/06_ConnectedRoutes"
browser_source="$browser_directory/BrowserConnectedRoutes.lpr"
browser_app="$browser_directory/browser_connected_routes_app.pas"
shared_demo="$browser_directory/connected_routes_demo.pas"
browser_html="$browser_directory/index.html"
browser_css="$browser_directory/connectedroutes.css"
output_directory="$repository_root/build/browser/connectivity"
unit_output_directory="$output_directory/units"
web_output_directory="$output_directory/www"
staged_javascript="$web_output_directory/BrowserConnectedRoutes.js"

for required_file in "$browser_source" "$browser_app" "$shared_demo" "$browser_html" "$browser_css"; do
  if test ! -f "$required_file"; then
    printf 'Required Connected Routes browser file was not found: %s\n' "$required_file" >&2
    exit 1
  fi
done
...
printf "Building Connected Routes with '%s'.\n" "$compiler"
...
for staged_asset in "$web_output_directory/index.html" "$web_output_directory/connectedroutes.css"; do
  if test ! -s "$staged_asset"; then
    printf 'Staged Connected Routes asset is empty: %s\n' "$staged_asset" >&2
    exit 1
  fi
done
printf "Connected Routes staged in '%s'.\n" "$web_output_directory"
