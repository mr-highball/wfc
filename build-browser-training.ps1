...
$browserSource = Join-Path $browserDirectory 'BrowserTrainingStudio.lpr'
$browserApp = Join-Path $browserDirectory 'browser_training_studio_app.pas'
$browserPresets = Join-Path $browserDirectory 'training_studio_presets.pas'
$browserHtml = Join-Path $browserDirectory 'index.html'
$browserCss = Join-Path $browserDirectory 'trainingstudio.css'
$outputDirectory = Join-Path $repositoryRoot 'build/browser/training'
$unitOutputDirectory = Join-Path $outputDirectory 'units'
$webOutputDirectory = Join-Path $outputDirectory 'www'
$stagedJavaScript = Join-Path $webOutputDirectory 'BrowserTrainingStudio.js'

foreach ($requiredFile in @(
    $browserSource, $browserApp, $browserPresets, $browserHtml, $browserCss)) {
  if (-not (Test-Path -LiteralPath $requiredFile -PathType Leaf)) {
    throw "Required Training Studio browser file was not found: $requiredFile"
  }
}
...
Write-Host "Building Training Studio with '$Compiler'."
...
foreach ($stagedAsset in @(
    ...
    (Join-Path $webOutputDirectory 'trainingstudio.css'))) {
  if ((Get-Item -LiteralPath $stagedAsset).Length -eq 0) {
    throw "Staged Training Studio asset is empty: $stagedAsset"
  }
}

Write-Host "Training Studio staged in '$webOutputDirectory'."
