...
browser_directory="$repository_root/examples/music/07_VoiceStudio"
browser_source="$browser_directory/BrowserVoiceStudio.lpr"
browser_app="$browser_directory/browser_voice_studio_app.pas"
studio_owner="$browser_directory/voice_studio_stream.pas"
browser_html="$browser_directory/index.html"
browser_css="$browser_directory/voicestudio.css"
output_directory="$repository_root/build/browser/voices"
unit_output_directory="$output_directory/units"
web_output_directory="$output_directory/www"
staged_javascript="$web_output_directory/BrowserVoiceStudio.js"
...
do
  if test ! -f "$required_file"; then
    printf 'Required Voice Studio browser file was not found: %s\n' \
      "$required_file" >&2
    exit 1
  fi
done
...
printf "Building Voice Studio with '%s'.\n" "$compiler"
"$compiler" "$@" \
  -B \
  -Tbrowser \
  -Mdelphi \
...
for staged_asset in \
  ...
  "$web_output_directory/voicestudio.css"
do
  if test ! -s "$staged_asset"; then
    printf 'Staged Voice Studio asset is empty: %s\n' "$staged_asset" >&2
    exit 1
  fi
done

printf "Voice Studio staged in '%s'.\n" "$web_output_directory"
