...
browser_directory="$repository_root/examples/text/03_PassComposition"
browser_source="$browser_directory/BrowserTextPassComposition.lpr"
browser_html="$browser_directory/index.html"
browser_css="$browser_directory/browsertextpasses.css"
output_directory="$repository_root/build/browser/text-passes"
unit_output_directory="$output_directory/units"
web_output_directory="$output_directory/www"
staged_javascript="$web_output_directory/BrowserTextPassComposition.js"

for required_file in "$browser_source" "$browser_html" "$browser_css"; do
  if test ! -f "$required_file"; then
    printf 'Required browser text demo file was not found: %s\n' \
      "$required_file" >&2
    exit 1
  fi
done
...
printf "Building the browser text workbench with '%s'.\n" "$compiler"
"$compiler" "$@" \
  -B \
  -Tbrowser \
  -Mdelphi \
...
for staged_asset in \
  ...
  "$web_output_directory/browsertextpasses.css"; do
  if test ! -s "$staged_asset"; then
    printf 'Staged browser text asset is empty: %s\n' "$staged_asset" >&2
    exit 1
  fi
done

printf "Browser text workbench staged in '%s'.\n" "$web_output_directory"
