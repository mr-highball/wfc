...
program wfc_pipeline_session_evidence_test;
{$mode delphi}{$H+}
uses {$IFDEF PAS2JS}wfc_browser_test_host,{$ENDIF}
  ...
  wfc_pipeline_session,wfc_session_oracle_helpers,wfc_session_evidence_checks,wfc_text_codec;
...
procedure TestPortableTextAndRetainedOwners;
var M: TWfcPipelineModel; R: TWfcPipelineRun; Resources: TWfcPipelineResources;
  Passes: TWfcPipelinePasses; Locks: TWfcPipelineCellLocks; Domains: TWfcPipelineCellDomains;
  P: TWfcPipelinePreparation; Input: TWfcPipelineInputPlan; B: TWfcPipelineInputBinding;
  Session: TWfcPipelinePreparedSession; O: TWfcPipelineSessionOutcome;
  Edit: TWfcPipelineSessionEditOutcome; Value,TraceValue: TWfcModelToken;
  Report: TGraphSolveReport; I: Integer; Found: Boolean;
begin
  Value:=WfcTextDecodeToken('caf%C3%A9%2C%3D%25%0D%0A','unicode evidence fixture');
  SetLength(Resources,1); Resources[0]:=RulesResource('accented',[Value,'quiet']);
  SetLength(Passes,1); Passes[0]:=MakeWfcPipelinePass('notes',wppvPublic,gpmOverlay,-1,wpakRules,0,False,wseWhole);
  M:=TWfcPipelineModel.Create(MakeWfcPipelineMetadata('text evidence','MIT','',''),1,False,rmBottomUp,Resources,Passes,nil,nil,nil);
  SetLength(Locks,1); Locks[0]:=MakeWfcPipelineCellLock(0,1,0,0,Value);
  { The core reports removed values: excluding the accented value here forces
    its actual graph text into the trace, while the second cell retains it. }
  SetLength(Domains,1); Domains[0]:=MakeWfcPipelineCellDomain(0,0,0,0,Tokens(['quiet']));
  R:=TWfcPipelineRun.Create(M,2,1,1,High(Cardinal),wpssOneWay,32,0,True,Locks,Domains);
  P:=nil; Input:=nil; B:=nil; Session:=nil; O:=nil; Edit:=nil;
  try
    P:=TWfcPipelinePreparation.Create(M,R); Input:=P.PrepareInputs(R);
    B:=TWfcPipelineInputBinding.CreateEditable(P,Input,ReplacementLimits);
    Session:=TWfcPipelinePreparedSession.Create(M,R,ReplacementLimits,OutcomeLimits);
    O:=Session.ExecuteInitial; CheckOutcome(O,Session,B,M,R,nil,True);
    Report:=O.CopySolveReport; Found:=False;
    for I:=0 to High(Report.Trace) do
    begin
      {$IFDEF PAS2JS}TraceValue:=Report.Trace[I].Value;
      {$ELSE}TraceValue:=UTF8Encode(UnicodeString(Report.Trace[I].Value));{$ENDIF}
      if TraceValue=Value then Found:=True;
    end;
    Check(Found,'fixture contains actual accented/control-text graph trace values');
    Edit:=Session.ApplyInputs(R); CheckEditEvidence(Edit);
    CheckEvidenceEnvelopeRejections(O,Edit);
    FreeAndNil(Session); FreeAndNil(B); FreeAndNil(Input); FreeAndNil(P); FreeAndNil(R); FreeAndNil(M);
    CheckOutcomeEvidence(O); CheckEditEvidence(Edit);
  finally Edit.Free; O.Free; Session.Free; B.Free; Input.Free; P.Free; R.Free; M.Free; end;
end;
...
begin
  ...
  TestPortableTextAndRetainedOwners;
  CheckInvalidEvidenceInputs;
  WriteLn('Prepared session evidence and independent oracle: ',OracleChecks,' checks passed.');
  WriteLn('Complete evidence documents: ',EvidenceDocuments);
end.
