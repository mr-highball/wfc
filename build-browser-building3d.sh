...
common_directory="$repository_root/examples/3D/common"
browser_directory="$repository_root/examples/3D/03_BrowserBuilding"
browser_source="$browser_directory/BrowserBuilding.lpr"
browser_html="$browser_directory/index.html"
browser_css="$browser_directory/browserbuilding.css"
output_directory="$repository_root/build/browser/building3d"
unit_output_directory="$output_directory/units"
web_output_directory="$output_directory/www"
staged_javascript="$web_output_directory/BrowserBuilding.js"

for required_file in "$browser_source" "$browser_html" "$browser_css"; do
  if test ! -f "$required_file"; then
    printf 'Required Building 3D browser file was not found: %s\n' "$required_file" >&2
    exit 1
  fi
done
...
printf "Building the Building 3D browser workbench with '%s'.\n" "$compiler"
"$compiler" "$@" \
  -B -Tbrowser -Mdelphi -Jc -Jirtl.js \
...
for staged_asset in "$web_output_directory/index.html" "$web_output_directory/browserbuilding.css"; do
...
printf "Building 3D browser workbench staged in '%s'.\n" "$web_output_directory"
